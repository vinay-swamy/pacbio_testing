...
model_d={'random_forest' : rf , 'xgbc': xgb, 'dense': dense}
##############
...
if mode == 'file':
    with open(outdir+'model_results.csv', 'w+') as model_res_file:
        if args.input_type == 'skl':
            data_list=load_snakemake_data(args.input_file, lab_file)
            ...
            data_list = load_merge(lab_file, args.input_file)
        ...
        trial=Experiment(data_list, c_model_dict, outdir)
        if args.exp_sel == 'all':
            print('running all')
